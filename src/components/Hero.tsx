...
const Hero = () => {
  ...
  const stablecoins = [
    { name: 'USDC', position: 'top-20 left-20', delay: '0s', color: 'from-blue-500 to-blue-600' },
    { name: 'USDT', position: 'top-40 right-16', delay: '1s', color: 'from-green-500 to-green-600' },
    { name: 'DAI', position: 'bottom-32 left-16', delay: '2s', color: 'from-yellow-500 to-orange-500' },
    { name: 'BUSD', position: 'bottom-20 right-24', delay: '3s', color: 'from-yellow-400 to-yellow-500' },
  ];
      ...
      {stablecoins.map((coin, index) => (
        <div
          ...
          className={`absolute ${coin.position} animate-drift transition-all duration-300 cursor-pointer ${
            hoveredCoin === coin.name ? 'opacity-80 scale-110' : 'opacity-30 hover:opacity-60'
          }`}
          style={{ animationDelay: coin.delay }}
          onMouseEnter={() => setHoveredCoin(coin.name)}
          onMouseLeave={() => setHoveredCoin(null)}
        >
          <div className={`w-12 h-12 bg-gradient-to-br ${coin.color} rounded-full flex items-center justify-center text-text-primary text-xs font-bold shadow-lg ${
            hoveredCoin === coin.name ? 'shadow-2xl shadow-electric-teal/50' : ''
          }`}>
            {coin.name}
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
          ...
          <div className="flex-1 text-center lg:text-left max-w-2xl">
            <div className="mb-6">
              ...
              <p className="text-xl text-text-muted mb-8 leading-relaxed">
                ...
                Seamlessly manage stablecoins and spend anywhere with our innovative card solution.
              </p>
              ...
              <div className="inline-flex items-center px-4 py-2 rounded-full bg-electric-teal/10 border border-electric-teal/20 mb-8">
                ...
                <span className="text-sm text-electric-teal font-medium">Mobile App Coming Soon</span>
              </div>
            </div>

            {/* CTAs */}
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <button className="group px-8 py-4 bg-gradient-to-r from-neon-blue to-electric-teal text-text-primary font-semibold rounded-2xl hover:shadow-2xl hover:shadow-neon-blue/25 transition-all duration-300 transform hover:scale-105 animate-pulse-glow">
          ...
          <div className="flex-1 flex justify-center lg:justify-end">
            <div className="relative">
              <div 
                className={`relative w-80 h-48 cursor-pointer transform-gpu transition-transform duration-700 ${isCardFlipped ? 'rotate-y-180' : ''}`}
                ...
                <div className="absolute inset-0 w-full h-full backface-hidden">
                  <div className="w-full h-full bg-gradient-to-br from-graphite via-graphite to-jet-black rounded-2xl shadow-2xl border border-electric-teal/20 p-6 animate-float">
                    <div className="flex justify-between items-start mb-8">
                      <div className="text-text-primary font-bold text-xl">PULSE</div>
                    ...
                    <div className="flex justify-between items-end">
                      <div>
                        <div className="text-text-muted text-xs mb-1">Card Holder</div>
                        <div className="text-text-primary text-sm font-semibold">EMMY.PULSE</div>
                      </div>
                      <div>
                        <div className="text-text-primary text-sm font-mono">•••• •••• •••• 1234</div>
                      </div>
                    </div>
                    
                    <div className="flex justify-between items-end mt-4">
                      <div>
                        <div className="text-text-muted text-xs mb-1">Valid Thru</div>
                ...
                <div className="absolute inset-0 w-full h-full backface-hidden rotate-y-180">
                  <div className="w-full h-full bg-gradient-to-br from-jet-black via-graphite to-graphite rounded-2xl shadow-2xl border border-neon-blue/20 p-6">
                    ...
                    <div className="bg-text-primary h-8 rounded mb-4 flex items-center px-3">
                      <div className="text-jet-black text-xs italic">Emmy.pulse</div>
                    </div>
